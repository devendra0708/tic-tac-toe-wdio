...
export default class BasePage {
  ...
  protected async waitForNativeConfirm(timeoutMsg: string, timeout = 5000): Promise<string> {
    let text = '';
  ...
  protected async stubConfirm(accept: boolean) {
    await browser.execute((shouldAccept) => {
      (window as unknown as { __lastConfirm: string | null }).__lastConfirm = null;
      window.confirm = (msg?: string) => {
        (window as unknown as { __lastConfirm: string | null }).__lastConfirm = String(msg ?? '');
        return shouldAccept;
      };
    }, accept);
  }
