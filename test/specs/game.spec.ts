...
describe('Game', () => {
  ...
  it('[GAME-006] shows computer-thinking after a human move', async () => {
    ...
    await expect(GamePage.status).toHaveAttribute('data-status', 'computer-thinking');
  });
  ...
  it('[GAME-017] Hint is disabled while computer thinks and after game over', async () => {
    ...
    await GamePage.waitForStatus(['your-turn', 'human', 'computer', 'draw'], 10000);
  ...
  it('[GAME-019] double-clicking an empty cell places only one X', async () => {
    ...
    await browser.execute(() => {
      const cell = document.querySelector('[data-testid="cell-4"]') as HTMLElement;
      cell.click();
      cell.click();
    });
    await GamePage.waitForStatus(['your-turn', 'human', 'computer', 'draw'], 10000);
